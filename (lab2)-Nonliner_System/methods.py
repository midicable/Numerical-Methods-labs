...
def GaussSeidelMethod(x0, y0):
    ...
    print(x_curr_GaussSeidel, y_curr_GaussSeidel)
    print(func1(x_curr_GaussSeidel, y_curr_GaussSeidel))
    print(func2(x_curr_GaussSeidel, y_curr_GaussSeidel))
#---------------------------------------
...
def SecantMethod(x0, y0, eps):
    v0 = np.array([[x0], [y0]])
    J = np.array([[func1XPartialDerivative(x0, y0), func1YPartialDerivative(x0, y0)],
                  [func2XPartialDerivative(x0, y0), func2YPartialDerivative(x0, y0)]])

    delta = np.linalg.inv(J).dot(np.array([[(-1) * func1(v0[0][0], v0[1][0])],
                                           [(-1) * func2(v0[0][0], v0[1][0])]]))
    v1 = v0 + delta
    while np.linalg.norm(v1 - v0, ord=np.inf) > eps:
        J = np.array([[(func1(v1[0][0], v1[1][0]) - func1(v0[0][0], v1[1][0])) / (v1[0][0] - v0[0][0]),
        ...
        v2 = v1 - np.linalg.inv(J).dot(np.array([[(-1) * func1(v0[0][0], v0[1][0])],
                                                 [(-1) * func2(v0[0][0], v0[1][0])]]))
